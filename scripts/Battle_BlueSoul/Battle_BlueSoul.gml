function Battle_BlueSoul(inst){
	var SPD = 2//Player_GetSpeedTotal()
	...
	if image_angle == DIR.DOWN {
		...
		var IsInsideBoard = Battle_IsBoardTouchTheEdge(1);
		var IsInsidePlatformExtra = InstanceQuadrilateralPosition(battle_platform, 1)
		var IsInsidePlatform = InstanceQuadrilateralPosition(battle_platform)
		...
		if IsInsideBoard[state] {
			if (move < 0) {
			    ...
			    if (!Input_IsDirect(input_jump)) {
			        // 平滑过渡到0而不是直接设为0
			        move = lerp(move, 0, 0.5);
			    }
    
			    // 防止从负值直接跳到正值
			    if (move > 0) {
			        move = lerp(0, move, 0.5); // 半平滑过渡
			    }
			} else if (move < _gravity_fall_max) {
			    var fallFactor = move / _gravity_fall_max; // 0到1的进度
			    move += _gravity_fall * (0.5 + fallFactor * 0.5); // 从50%到100%重力
			}
		...
		//随板子移动
		var PlatformList = InstanceGetList(battle_platform)
